 ...
 * @since 1.0
 */
public final class QvtdStoreUnitResolver implements QvtdUnitResolver {
	 ...
	 */
	public QvtdStoreUnitResolver(UnitStore store) {
		this(store, null);
	}
	 ...
	 */
	public QvtdStoreUnitResolver(UnitStore store, EPackage.Registry packages) {
	...
	public Optional<QvtdUnit> resolveUnit(String qualifiedName) {
		...
		try {
			...
			if (compiled.isPresent() && compiled.get() instanceof PackagedUnit packaged
					&& packaged.document().getUnit() instanceof RelationalTransformation root) {
				materializer.materialize(packaged,
						packages == null ? new UnitResourceSet() : new UnitResourceSet(packages));
				return Optional.of(new QvtdUnit.CompiledUnit(qualifiedName, root));
			}
			...
			if (source.isPresent() && source.get() instanceof Unit.Source text) {
				return Optional.of(new QvtdUnit.SourceUnit(qualifiedName, text.uri(), text.source()));
			}
			return Optional.empty();
