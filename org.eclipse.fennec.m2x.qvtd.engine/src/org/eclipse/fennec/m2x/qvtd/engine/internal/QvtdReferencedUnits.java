 ...
 * {@code QvtdUnit.ResourceUnit}. Loading and binding happen here, in a context over the
 ...
 * @since 1.0
 */
final class QvtdReferencedUnits {
	...
	QvtdReferencedUnits(EPackage.Registry packages) {
	 ...
	 * @throws QvtdParseException if the reference cannot be loaded, or names no QVT-R unit
	 */
	QvtdUnit dereference(QvtdUnit unit) throws QvtdParseException {
		if (unit instanceof QvtdUnit.CompiledUnit compiled
			...
			try {
				...
				throw new QvtdParseException("Cannot resolve import '" + compiled.qualifiedName() + "': "
						+ failure.getMessage(), failure);
			}
			return unit;
		}
		if (!(unit instanceof QvtdUnit.ResourceUnit reference)) {
			return unit;
		}
		EObject root;
		try {
			...
			throw new QvtdParseException("Cannot resolve import '" + reference.qualifiedName() + "': "
					+ failure.getMessage(), failure);
		}
		EObject ast = root instanceof CompiledUnit document ? document.getUnit() : root;
		if (ast instanceof RelationalTransformation transformation) {
			return new QvtdUnit.CompiledUnit(reference.qualifiedName(), transformation);
		}
		throw new QvtdParseException("Cannot resolve import '" + reference.qualifiedName() + "': "
				+ reference.uri() + " holds no QVT-R unit but "
