...
| D47 | Der geparste AST ist ein persistierbares Modell — Containment statt Save-API | `type`-Referenzen (ocl, m2t) und `configProperty`/`intermediateProperty` (qvto) sind Containments; Typinstanzen werden nie geteilt, sondern kopiert (`copyType` = `EcoreUtil.copy`). Der QVT-O-Parser hängt ModelTypes in `eClassifiers`, Model-Parameter-Variablen in `ownedVariable` und sammelt übrige Satelliten in einem Reparatur-Pass ein (#127). Siehe DR-D47. |
...
### DR-D47: Der geparste AST ist ein persistierbares Modell — Containment statt Save-API (#127)

**Entscheidung:** `QvtoEngine.parse(...)` liefert einen Baum, der unverändert in eine `Resource` gelegt und als XMI gespeichert werden kann. Erreicht wird das über das Metamodell und den Parser, nicht über eine eigene `save`/`load`-API: die `type`-Referenzen des OCL-Modells (`OclExpression.type`, `Variable.type`, `CollectionType.elementType`, `MapType.keyType`/`valueType`, `TypeExp.referredType`, `TupleLiteralPart.type`, `TuplePart.type`) und die `returnType`-Referenzen des M2T-Modells sind Containments, ebenso `Module.configProperty` und `OperationalTransformation.intermediateProperty`. Der QVT-O-Parser hängt ModelTypes zusätzlich in `eClassifiers` (Module ist ein `EPackage`, ModelType eine `EClass` — `usedModelType` bleibt der Spec-Querverweis), legt die Variable eines Model-Parameters in `ownedVariable` ab und verknüpft sie über `VarParameter.representedParameter`.

**Warum Kopien statt geteilter Typinstanzen:** Mit Containment stiehlt die zweite Zuweisung derselben Typinstanz dem ersten Besitzer den Typ — mitten im Parsen, wo die Typen die Namensauflösung tragen. `copyType` ist deshalb `EcoreUtil.copy` (vorher: nur Classifier- und CollectionType, sonst `null`), und jede Stelle, die einen Typ von einem anderen Besitzer übernimmt, kopiert ihn. Geteilt wird nie.

**Warum zusätzlich ein Reparatur-Pass:** Satelliten entstehen an zu vielen Stellen, um jede einzeln einzuhängen — Environment-Variablen (`self`, Parameter, `result`), desugarte Iterator-, Extent- und `this`-Variablen, Inline-Libraries und Linker-Stubs, die synthetische `EDataType` eines primitiven Kontexttyps, das synthetische `EAttribute` einer erst zur Laufzeit auflösbaren Property. `containSatellites` läuft am Ende von `visitCompilationUnit` bis zum Fixpunkt (angehängte Variablen ziehen über ihre `ownedInit`-Bäume weitere Satelliten nach): hängelose Variablen kommen in die nächstliegende `OperationBody.variable`, sonst in `Module.ownedVariable`; hängelose Module aus `importedModule`/`refined` werden `eSubpackages`; paketlose Classifier landen in `eClassifiers`; synthetische Features und Operationen in einer Sammel-`EClass` `_SYNTHETIC_MEMBERS`. Objekte mit `eResource()` oder `eContainer()` — registrierte Metamodelle, deren Classifier — fasst der Pass nie an.

**Folge für die Engine:** `ownedVariable` enthält jetzt auch die Model-Parameter-Variablen; die Initialisierung der Modul-Properties überspringt Variablen, auf die ein `representedParameter` zeigt — sonst würde `env.define(name, null)` die bereits gebundenen Extents überschreiben.

**Grenzen:** Ein `ModuleImport` auf ein echtes Fremdmodul serialisiert als href — jedes Modul ist seine eigene Resource, gespeichert wird pro Unit. Und die Metamodelle selbst reisen nicht mit: Laden setzt dieselben nsURIs in der Registry bzw. auflösbare Resource-URIs voraus. OCL, M2T und QVT-R profitieren von der Typ-Containment-Änderung; ihre Environment-Variablen (etwa `self` eines nackten OCL-Ausdrucks) haben dort weiterhin kein Containment-Zuhause — Follow-up, falls auch diese ASTs persistiert werden sollen.
