...
|--------|-------------|--------|
| `org.eclipse.fennec.m2x.unit` | Unit abstraction, store, fingerprint and prepared-context contracts, compiled-unit manifest metamodel | Contracts |
| `org.eclipse.fennec.m2x.unit.tests` | 26 tests, 0 failures | Contracts |

The three language APIs depend on this bundle, never the other way round, so
`QvtoUnit`, `QvtdUnit` and `M2tUnit` share one base type without a cycle.
...
| `org.eclipse.fennec.m2x.qvto.tests` | 1,160 tests (5 disabled), 0 failures | Done |
...
| `org.eclipse.fennec.m2x.qvtd.tests` | 193 tests (10 disabled), 0 failures | Done |
...
| `org.eclipse.fennec.m2x.m2t.tests` | 343 tests (8 disabled), 0 failures | Done |
...
The per-bundle counts above are a snapshot, and the honest way to read them is with a date: **6,069 tests and 47 OSGi tests, 0 failures, as of 2026-08-28**. `./gradlew build` and `./gradlew testOSGi` are what produce that number — if it differs from the tables, the tables are the stale ones.
