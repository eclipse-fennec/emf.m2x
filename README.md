...
|--------|-------------|--------|
| `org.eclipse.fennec.m2x.unit` | `CompiledUnit` metamodel (manifest, script, satellites, embedded units), `SatelliteCollector` + `UnitPackager` behind `compile()`, store / fingerprint / prepared-context contracts | In progress |
| `org.eclipse.fennec.m2x.unit.tests` | 44 tests, 0 failures | In progress |
...
the storable document (#137). Fingerprint, store and dependency binding follow (#138–#142).
...
| `org.eclipse.fennec.m2x.qvto.tests` | 1,169 tests (1 disabled), 0 failures | Done |
...
| `org.eclipse.fennec.m2x.qvtd.tests` | 201 tests (2 disabled), 0 failures | Done |
...
| `org.eclipse.fennec.m2x.m2t.tests` | 359 tests (1 disabled), 0 failures | Done |
...
The per-bundle counts above are a snapshot, and the honest way to read them is with a date: **6,153 tests and 47 OSGi tests, 0 failures, as of 2026-08-28**. `./gradlew build` and `./gradlew testOSGi` are what produce that number — if it differs from the tables, the tables are the stale ones.
