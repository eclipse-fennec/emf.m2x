...
under `embed`, `pin` or `rebind` (#139). Store, prepared context and load-time validation
follow (#140–#142).
...
| `org.eclipse.fennec.m2x.qvto.tests` | 1,190 tests (1 disabled), 0 failures | Done |
...
| `org.eclipse.fennec.m2x.qvtd.tests` | 214 tests (2 disabled), 0 failures | Done |
...
| `org.eclipse.fennec.m2x.m2t.tests` | 372 tests (1 disabled), 0 failures | Done |
...
The per-bundle counts above are a snapshot, and the honest way to read them is with a date: **6,215 tests and 47 OSGi tests, 0 failures, as of 2026-08-28**. `./gradlew build` and `./gradlew testOSGi` are what produce that number — if it differs from the tables, the tables are the stale ones.
